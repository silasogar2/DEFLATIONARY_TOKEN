...
contract ShowToken is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "SHOWTOKEN";
    string private _symbol = "SHOWTOKEN";
    ...
    function swapTokens(uint256 amount) private lockTheSwap {
        ...
        if (transferredBalance > 0) {
            transferBNBtoAddress(
                developersAddress,
                transferredBalance.mul(developersPercentage).div(10 ** 2)
            );
            transferBNBtoAddress(
                charityAddress,
                transferredBalance.mul(charityPercentage).div(10 ** 2)
            );
        }
    }
    ...
    function buyBackTokens() private lockTheSwap {
        ...
        if (buyBackEnabled == true) {
            if (balance > 0) swapBNBForTokens(balance);
        }
    }
    ...
    function _getTax(uint256 amount) private view returns (uint256, uint256) {
        uint256 taxAmount = amount.mul(taxFee).div(10 ** 2);
    ...
    function viewAddresses() public view onlyOwner returns (address, address) {
        return (developersAddress, charityAddress);
    }

    function setAccountPercentages(
        uint256 _taxFee,
        uint256 _charityPercentage,
        uint256 _developersPercentage,
    ...
    function startAutoDispensal() public onlyOwner {
        enableTrading = true;
    }

    function stoptAutoDispensal() public onlyOwner {
        enableTrading = false;
    }
    ...
    function autoSwapAndBuyBack() public {
        uint256 contractTokenBalance = balanceOf(address(this));
        if (contractTokenBalance > 0) swapTokens(contractTokenBalance);
        buyBackTokens();
    }
